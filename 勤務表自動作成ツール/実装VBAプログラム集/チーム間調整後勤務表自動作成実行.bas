...
Private ws As Worksheet
  'シート「チーム間調整前総合勤務表」エラー判定結果格納変数
Private error_judge As Boolean
...
Private disable_hope As Boolean
'書き込み用のAチームの人数格納変数
Private A_team_people_num As Integer
Private offset As Integer
Private human_offset As Integer
Private team_people_num As Integer
'ループ変数の定義
...
Function high_speeding(ByVal start_end As Boolean) 'VBA開始の時はTrue, 終了の時はFalseでVBA高速化を開始・終了する
  ...
  End If
End Function
Function A_B_change(ByVal A_or_B As String) As Boolean
  If A_or_B = "A" Then
    offset = 19
    If number_of_people <> 0 Then
      human_offset = 0
      team_people_num = A_team_people_num
    End If
  Else
    If offset = 19 Then
      offset = 79
      If number_of_people <> 0 Then
        human_offset = A_team_people_num
        team_people_num = number_of_people - A_team_people_num
      End If
      A_B_change = True
    End If
  End If
End Function
Function team_name(ByVal people_index As Integer) As String
  If people_index > A_team_people_num Then
    team_name = "Bチーム"
  Else
    team_name = "Aチーム"
  End If
...
'パラメータ設定の完了確認
  ...
  If ws.Cells(1, 4) <> "二交代制" And ws.Cells(1, 4) <> "三交代制" Then '二交代制, 三交代制のどちらも入力されていない場合
    MsgBox "A,Bチーム用勤務希望表の「勤務制度」に, " & vbCrLf & "「二交代制」,又は,「三交代制」と入力して下さい ！" & vbCrLf & "尚, 両チームの勤務制度は同じにして下さい!", _
  ...
  If ws.Cells(2, 4) = "" Then
    MsgBox "A,Bチーム用勤務希望表で日勤の人数設定をして下さい！", _
    Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
    Exit Function
  ElseIf VarType(ws.Cells(2, 4)) <> 5 Then
    MsgBox "A,Bチーム用勤務希望表の日勤の人数設定の欄には数字を入力して下さい！" _
    & vbCrLf & "尚, 文字列で入力しないで下さい！", _
    Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
    Exit Function
  '勤務表作成年月確認
  ElseIf Not ws.Cells(9, 4) Like "20*/*/*" Then
    MsgBox "何年何月の勤務表を作成するかをA,Bチーム用勤務希望表のD4セルに入力して下さい！" _
    & vbCrLf & "尚, 両チームの勤務表の対象月は同じにして下さい!" _
    & vbCrLf & "又, 形式は以下のように書いて下さい！" _
  ...
  If ws.Cells(1, 4) = "二交代制" Then
    ...
    For check_cell_num = 3 To 4
      If ws.Cells(check_cell_num, 4) = "" Then
        MsgBox "A,Bチーム用勤務希望表で当直の" & check_sub_title(check_sub_title_num) & "をして下さい！", _
        Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
        Exit Function
      ElseIf VarType(ws.Cells(check_cell_num, 4)) <> 5 Then
        MsgBox "A,Bチーム用勤務希望表の当直の" & check_sub_title(check_sub_title_num) & "の欄には数字を入力して下さい！" _
        & vbCrLf & "尚, 文字列で入力しないで下さい！", _
        Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
        Exit Function
      End If
    ...
    '1カ月の当直の最低回数の設定完了確認
      'Aチーム確認
    Call A_B_change("A")
B_team_return_point_duty:
    For check_people_num = offset To offset + 2 * (team_people_num - 1) Step 2
      If ws.Cells(check_people_num, 6) = "" Then
        MsgBox team_name((check_people_num - offset) / 2 + 1 + human_offset) & Names((check_people_num - offset) / 2 + 1 + human_offset) & "さんの1カ月あたりの当直の最低回数を設定して下さい！", _
        Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
        Exit Function
      ElseIf VarType(ws.Cells(check_people_num, 6)) <> 5 Then
        MsgBox team_name((check_people_num - offset) / 2 + 1 + human_offset) & Names((check_people_num - offset) / 2 + 1 + human_offset) & "さんの1カ月あたりの当直の最低回数の欄には数字を入力して下さい！" _
        & vbCrLf & "尚, 文字列で入力しないで下さい！", _
        Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
        Exit Function
      End If
    Next
    If A_B_change("B") Then
      GoTo B_team_return_point_duty
    End If
  ...
  ElseIf ws.Cells(1, 4) = "三交代制" Then
    ...
    For check_cell_num = 5 To 8
      If ws.Cells(check_cell_num, 4) = "" Then
        MsgBox "A,Bチーム用勤務希望表で" & check_title_for_three_change(check_title_num_for_three_change) & "の" & check_sub_title(check_sub_title_num) & "をして下さい！", _
        Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
        Exit Function
      ElseIf VarType(ws.Cells(check_cell_num, 4)) <> 5 Then
        MsgBox "A,Bチーム用勤務希望表の" & check_title_for_three_change(check_title_num_for_three_change) & "の" & check_sub_title(check_sub_title_num) & "の欄には数字を入力して下さい！" _
        & vbCrLf & "尚, 文字列で入力しないで下さい！", _
        Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
        Exit Function
      End If
    ...
    Call A_B_change("A")
B_team_return_point_night:
    check_title_num_for_three_change = 0
    For check_people_num = offset To offset + 2 * (team_people_num - 1) Step 2
      For check_cell_num = 7 To 8
        If ws.Cells(check_people_num, check_cell_num) = "" Then
          MsgBox team_name((check_people_num - offset) / 2 + 1 + human_offset) & Names((check_people_num - offset) / 2 + 1 + human_offset) & "さんの1カ月あたりの" & check_title_for_three_change(check_title_num_for_three_change) & "の最低回数を設定して下さい！", _
          Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
          Exit Function
        ElseIf VarType(ws.Cells(check_people_num, check_cell_num)) <> 5 Then
          MsgBox team_name((check_people_num - offset) / 2 + 1 + human_offset) & Names((check_people_num - offset) / 2 + 1 + human_offset) & "さんの1カ月あたりの" & check_title_for_three_change(check_title_num_for_three_change) & "の最低回数の欄には数字を入力して下さい！" _
          & vbCrLf & "尚, 文字列で入力しないで下さい！", _
          Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
          Exit Function
        End If
    ...
    Next
    If A_B_change("B") Then
      GoTo B_team_return_point_night
    End If
  End If
  
 '個人の1カ月あたりの休みの設定完了確認
  Call A_B_change("A")
B_team_return_point_holiday:
  For check_people_num = offset To offset + 2 * (team_people_num - 1) Step 2
    If ws.Cells(check_people_num, 9) = "" Then
      MsgBox team_name((check_people_num - offset) / 2 + 1 + human_offset) & Names((check_people_num - offset) / 2 + 1 + human_offset) & "さんの1カ月あたりの休みの最低回数を設定して下さい！", _
      Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
      Exit Function
    ElseIf VarType(ws.Cells(check_people_num, 9)) <> 5 Then
      MsgBox team_name((check_people_num - offset) / 2 + 1 + human_offset) & Names((check_people_num - offset) / 2 + 1 + human_offset) & "さんの1カ月あたりの休みの最低回数の欄には数字を入力して下さい！" _
      & vbCrLf & "尚, 文字列で入力しないで下さい！", _
      Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
      Exit Function
    End If
  Next
  If A_B_change("B") Then
    GoTo B_team_return_point_holiday
  End If
  
'エラー確認
  Call A_B_change("A")
B_team_return_point_error:
  For check_people_num = offset To offset + 2 * (team_people_num - 1) Step 2
    If ws.Cells(check_people_num, 52) <> "" Then
      MsgBox team_name((check_people_num - offset) / 2 + 1 + human_offset) & Names((check_people_num - offset) / 2 + 1 + human_offset) & "さんの希望欄でエラーが発生しています！", _
      Buttons:=vbCritical, Title:="希望入力者の方向けのエラー表示"
      Exit Function
    End If
  Next
  If A_B_change("B") Then
    GoTo B_team_return_point_error
  End If
...
Function day_work_classifying(ByVal Work_Name As String) As Variant '「日勤, 当直, 準夜勤, 深夜勤, 休みの方の中で」「月合計が余っている, かつ, 最大の方+他の条件」を探索する時にその方の要素数を取得する為に使う
  ...
  If how_work = "二交代制" Then
    ...
    For classify_people_num = 0 To number_of_people - 1
      '固定勤務希望でなければ,又は希望無効化がTrueになっていれば
      If hope_or_not(classify_people_num, j) = 0 Or disable_hope Then
...
Point_End:
  ...
  End If
End Function
Function 各シートチーム間調整勤務表書き込み(ByVal write_sheet_name As String, ByVal disabling_hope As Boolean)
'Private変数の再初期化
  'グローバル変数の定義
    '読み込み用シート指定
...
'読み込み時用変数
  ...
  '希望後回し勤務表自動作成時の書き込み時の照合用配列
  Dim save_request() As String
...
B_team_return_point_num_people:
  ...
  Next
  '名前が無かったら, 終了する
  If (offset = 19 And number_of_people = 0) Or (offset = 79 And number_of_people = A_team_people_num) Then
    error_judge = True
    Exit Function
  End If
  If A_B_change("B") Then
    team_people_num = 0
    'Aチーム(上の欄)の人数を数えた後, 格納しておく
    A_team_people_num = number_of_people
  ...
  '最初のみシート「チーム間調整前総合勤務表」のエラーを確認する
  If write_sheet_name = "希望優先チーム間調整後総合勤務表" Then
    '開始前の設定不足, エラー, 日数不足確認
  ...
  If how_work = "二交代制" Then
    ...
    'M3.1カ月の当直の余り, 不足日数表示
    Call A_B_change("A")
B_team_return_point_duty_around:
    For i = offset To offset + 2 * (team_people_num - 1) Step 2
      request_aroud_results_month(1, (i - offset) / 2 + human_offset) = ws.Cells(i, 43)
      request_aroud_results_month(2, (i - offset) / 2 + human_offset) = excess_miss(ws.Cells(i, 44))
    Next
    If A_B_change("B") Then
      GoTo B_team_return_point_duty_around
    End If
  ...
  ElseIf how_work = "三交代制" Then
    ...
    'M5.1カ月の深夜勤の余り, 不足日数表示
    Call A_B_change("A")
B_team_return_point_night_around:
    For i = offset To offset + 2 * (team_people_num - 1) Step 2
      request_aroud_results_month(1, (i - offset) / 2 + human_offset) = ws.Cells(i, 45)
      request_aroud_results_month(2, (i - offset) / 2 + human_offset) = excess_miss(ws.Cells(i, 46))
      request_aroud_results_month(3, (i - offset) / 2 + human_offset) = ws.Cells(i, 47)
      request_aroud_results_month(4, (i - offset) / 2 + human_offset) = excess_miss(ws.Cells(i, 48))
    Next
    If A_B_change("B") Then
      GoTo B_team_return_point_night_around
    End If
  End If
  ...
  Call A_B_change("A")
B_team_return_point_holiday_around:
  For i = offset To offset + 2 * (team_people_num - 1) Step 2
    request_aroud_results_month(0, (i - offset) / 2 + human_offset) = ws.Cells(i, 42)
    request_aroud_results_month(UBound(request_aroud_results_month, 1) - 1, (i - offset) / 2 + human_offset) = ws.Cells(i, 49)
    request_aroud_results_month(UBound(request_aroud_results_month, 1), (i - offset) / 2 + human_offset) = excess_miss(ws.Cells(i, 50))
  Next
  If A_B_change("B") Then
    GoTo B_team_return_point_holiday_around
  End If
  ...
  Call A_B_change("A")
B_team_return_point_request:
  For i = offset To offset + 2 * (team_people_num - 1) Step 2
    For j = 11 To 11 + number_of_day - 1
      work_content = ws.Cells(i, j)
      If how_work = "二交代制" Then
        If work_content <> "日" And work_content <> "休" And work_content <> "当" And work_content <> "明" Then
          'VBA高速化を終了し
          Call high_speeding(False)
          'エラーメッセージを出力し
          MsgBox team_name((i - offset) / 2 + 1 + human_offset) & Names((i - offset) / 2 + 1 + human_offset) & "さんの" & CStr(j - 11 + 1) & "日の勤務形式が不適切です！" & vbCrLf & "「日」「休」「当」「明」のどれかを記載して下さい！", _
          Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
          'Subプロジージャを終了する
          Exit Function
        End If
      ElseIf how_work = "三交代制" Then
        If work_content <> "日" And work_content <> "休" And work_content <> "準" And work_content <> "深" Then
          'VBA高速化を終了し
          Call high_speeding(False)
          'エラーメッセージを出力し
          MsgBox team_name((i - offset) / 2 + 1 + human_offset) & Names((i - offset) / 2 + 1 + human_offset) & "さんの" & CStr(j - 11 + 1) & "日の勤務形式が不適切です！" & vbCrLf & "「日」「休」「準」「深」のどれかを記載して下さい！", _
          Buttons:=vbCritical, Title:="設定者の方向けのエラー表示"
          'Subプロジージャを終了する
          Exit Function
        End If
      End If
      request_table((i - offset) / 2 + human_offset, j - 11) = work_content
      '字が赤かったら, プログラムで作成した勤務形式なので
      If Cells(i, j).Font.color = 255 Then
        hope_or_not((i - offset) / 2 + human_offset, j - 11) = 0
      '字が黒かったら,希望勤務形式なので
      Else
        hope_or_not((i - offset) / 2 + human_offset, j - 11) = 1
      End If
      remarks_table((i - offset) / 2 + human_offset, j - 11) = ws.Cells(i + 1, j)
    Next
  Next
  If A_B_change("B") Then
    GoTo B_team_return_point_request
  End If
  
  '希望後回し勤務表自動作成開始時には元の勤務表を書き込み時の照合用にコピーする
  If disabling_hope Then
    save_request = request_table
  End If
...
calculation_again_point:
  ...
  '書き込み用アクティブシート指定
  Set ws = ThisWorkbook.Sheets(write_sheet_name)
  'シートの保護の解除
  Call pro_unpro(False, write_sheet_name)
  Call A_B_change("A")
B_team_return_point_write:
  '削除
  Range(ws.Cells(offset, 11), ws.Cells(offset + 30 * 2 - 1, 41)).ClearContents
  '太字→普通の細さにする
  Range(ws.Cells(offset, 11), ws.Cells(offset + 30 * 2 - 1, 41)).Font.Bold = False
  '斜体→直体にする
  Range(ws.Cells(offset, 11), ws.Cells(offset + 30 * 2 - 1, 41)).Font.Italic = False
  '字の色を黒に変える
  Range(ws.Cells(offset, 11), ws.Cells(offset + 30 * 2 - 1, 41)).Font.color = RGB(0, 0, 0)
  If Not disabling_hope Then
    '書き込み
    For i = offset To offset + 2 * (team_people_num - 1) Step 2
      For j = 11 To 11 + number_of_day - 1
        '固定希望日ではなく, VBA上で自動作成した勤務形式ならば
        If hope_or_not((i - offset) / 2 + human_offset, j - 11) <> 1 Then
          '太字に変えて
        ...
        End If
        ws.Cells(i, j) = request_table((i - offset) / 2 + human_offset, j - 11)
        ws.Cells(i + 1, j) = remarks_table((i - offset) / 2 + human_offset, j - 11)
      Next
    Next
  Else
    '書き込み
    For i = offset To offset + 2 * (team_people_num - 1) Step 2
      For j = 11 To 11 + number_of_day - 1
        'VBA上で自動作成した勤務形式ならば
        If hope_or_not((i - offset) / 2 + human_offset, j - 11) = 0 Then
          '太字に変えて
          ws.Cells(i, j).Font.Bold = True
          '斜体に変えて
          ws.Cells(i, j).Font.Italic = True
          '字の色を赤に変える
          ws.Cells(i, j).Font.color = RGB(255, 0, 0)
        ElseIf save_request((i - offset) / 2 + human_offset, j - 11) <> request_table((i - offset) / 2 + human_offset, j - 11) Then
          '太字に変えて
          ws.Cells(i, j).Font.Bold = True
          '斜体に変えて
          ws.Cells(i, j).Font.Italic = True
          '字の色を紫に変える
          ws.Cells(i, j).Font.color = RGB(255, 0, 255)
        End If
        ws.Cells(i, j) = request_table((i - offset) / 2 + human_offset, j - 11)
        ws.Cells(i + 1, j) = remarks_table((i - offset) / 2 + human_offset, j - 11)
      Next
  ...
  End If
  If A_B_change("B") Then
    GoTo B_team_return_point_write
  End If
  
  '勤務表自動作成最終日時更新
  ws.Cells(19, 1) = Format(Now(), "YYYY年MM月DD日HH時MM分")
  'シートの保護の再有効化
  Call pro_unpro(True, write_sheet_name)
  'VBA高速化を終了し
  Call high_speeding(False)
End Function
Sub チーム間調整後勤務表自動作成実行()
  'Private変数の初期化
  Set ws = ThisWorkbook.Sheets("チーム間調整前総合勤務表")
  error_judge = False
  ...
  Call 各シートチーム間調整勤務表書き込み("希望優先チーム間調整後総合勤務表", False)
  If Not error_judge Then
    Call 各シートチーム間調整勤務表書き込み("希望後回しチーム間調整後総合勤務表", True)
  End If
  
  If Not error_judge Then
    '完了メッセージを表示する
    MsgBox "チーム間調整後の勤務表の作成が完了しました！" _
        & vbCrLf & "シート「希望優先チーム間調整後総合勤務表」と" _
        & vbCrLf & "シート「希望後回しチーム間調整後総合勤務表」をご確認下さい！", _
        Buttons:=vbInformation, Title:="実行者の方へのメッセージ"
  ElseIf (offset = 19 And number_of_people = 0) Or (offset = 79 And number_of_people = A_team_people_num) Then
    If offset = 19 Then
      err_team_name = "Aチーム"
    ...
    & vbCrLf & "④" & "この「チーム間調整勤務表自動作成実行」ボタンを再度, 押す", _
...
ErrLabel:
  'シートの保護の再有効化
  Call pro_unpro(True, "希望優先チーム間調整後総合勤務表")
  Call pro_unpro(True, "希望後回しチーム間調整後総合勤務表")
  'VBA高速化を終了し
  Call high_speeding(False)
  MsgBox "「チーム間調整勤務表自動作成実行」VBA実行中にエラーが発生しました！" _
  & vbCrLf & "シート「チーム間調整前総合勤務表」をご確認の上, 再度, 実行して下さい！" _
