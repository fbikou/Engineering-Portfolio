...
'Naming
  'number_of_peopleのループと, 最後の評価の際の1日毎の余り・不足人数配列の要素探索用のi
  Dim i As Integer
  '以下, デバッグ用変数
...
'exectuing
  ...
  'request_table_readingとhope_or_notと備考の格納
...
calculation_again_point:
  ...
  If how_work = "二交代制" Then
    ...
    For j = 0 To number_of_day - 1
      ...
      If request_aroud_results_day(0, j) > 0 Then
        While day_work_classifying("日")(1) = "有り"
  ...
  '削除
  Range(ws.Cells(19, 11), ws.Cells(78, 41)).ClearContents
  '太字→普通の細さにする
  Range(ws.Cells(19, 11), ws.Cells(78, 41)).Font.Bold = False
  '斜体→直体にする
  Range(ws.Cells(19, 11), ws.Cells(78, 41)).Font.Italic = False
  '字の色を赤→黒に変える
  Range(ws.Cells(19, 11), ws.Cells(78, 41)).Font.color = RGB(0, 0, 0)
  '書き込み
  For i = 19 To 19 + 2 * (number_of_people - 1) Step 2
    For j = 11 To 11 + number_of_day - 1
      '固定希望日ではなく, VBA上で自動作成した勤務形式ならば
      If hope_or_not((i - 19) / 2, j - 11) <> 1 Then
        '太字に変えて
      ...
      End If
      ws.Cells(i, j) = request_table((i - 19) / 2, j - 11)
      ws.Cells(i + 1, j) = remarks_table((i - 19) / 2, j - 11)
    Next
  Next
  
  '勤務表自動作成最終日時更新
  ws.Cells(19, 1) = "Aチーム:" & Format(Now(), "YYYY年MM月DD日HH時MM分")
  'シートの保護の再有効化
  ...
  '完了メッセージを表示する
  MsgBox "Aチーム単体での勤務表の作成が完了しました！" _
...
ErrLabel:
  ...
  'VBA高速化を終了する
  Call high_speeding(False)
  MsgBox "「Aチーム用勤務表自動作成実行」VBA実行中にエラーが発生しました！" _
